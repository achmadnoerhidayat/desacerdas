<?php
	...
	$fld_nm_tjwb=$_GET['txt1'];
	$fld_jbt_tjwb=$_GET['txt2'];
	$fld_alamat_tjwb=$_GET['txt3'];	
	$nama=$_GET['txt4'];	
	...
	$tgl_surat="";
		...
		$cari_kd=mysqli_query($koneksi,"SELECT no_pengaturan 
						FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='13'");
		$tm_cari=mysqli_fetch_array($cari_kd);
	...
	$cari_kd=mysqli_query($koneksi,"SELECT 
									...
									WHERE id_kel='$kd_kel' and id_kategori='13'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	...
	$html = '<head>
			...
			<div style="text-align: center; margin: 30pt 0;">
				<div style="text-decoration: underline; font-weight: bold;">SURAT KETERANGAN BELUM MENIKAH</div>		
				<div>'.$no_surat.'</div>				
			</div>

        <!-- Begin : Isi Surat -->
        <div style="margin-top: 30pt;">Yang bertanda tangan di bawah ini, '.$jabatan.' '.$fld_nama_kel.', '.$fld_nama_kec.' dengan ini menerangkan  :</div>
        ...
        <div style="text-indent: 40px; margin-top: 30pt;">
            Menyatakan dengan sebenarnya, bahwa hingga saat ini <u>belum pernah menikah</u> dengan siapapun baik secara adat, hukum Agama maupun hukum Negara dan benar- benar masih gadis.
        </div>
        <div style="text-indent: 40px; margin-top: 30pt;">
            Demikian Surat Keterangan Belum Menikah ini dikeluarkan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.
        </div>
...
$dompdf->stream('surat_keterangan_belum_menikah.pdf',array("Attachment"=>0));
//$dompdf->stream('surat_keterangan_belum_menikah.pdf');
?>
