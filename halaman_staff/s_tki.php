...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='28'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='28'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Izin Bekerja Ke Luar Negeri Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}

	if(isset($_POST['btnsimpan'])) {			
		$cbokel = $_POST['txtkdkel'];
		$txtnama = $_POST['txtnama'];
		$title="SURAT IZIN BEKERJA KE LUAR NEGERI ".$txtnama;	
		...
		$cbostatustki = $_POST['cbostatustki'];
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							fld_tki1, fld_tki2) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','28',
							'$txtnama','$cbopekerjaan','$txtalamat', 
							'$txtnama1','$cbopekerjaan1','$txtalamat1',
							'$txtnegara','$cbostatustki')");
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="flex items-center mt-8 intro-y">
        <h2 class="mr-auto text-lg font-medium">Layanan Surat - Form Buat Surat Izin Bekerja Keluar Negeri</h2>
    </div>
        ...
        <div class="p-5 mt-2 box intro-y">
            <div class="flex">
                <div class="flex-row items-center w-1/3 mr-2">
                    ...
                    <div class="w-full mt-4">
                        <label class="block mb-2">Negara Tujuan</label>
						<input type="text" class="input w-full border mt-2" id="txtnegara" name="txtnegara" autocomplete="off" required>
                    </div>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>
                ...
                <div class="flex-row items-center w-1/3 ml-2">
                    <div class="w-full pb-3 border-b border-gray-300">
                        <h4 class="mr-auto text-lg font-medium">Biodata Penanggung Jawab</h4>
                    </div>
                    ...
                    <div class="w-full mt-4">
                        <label class="block mb-4">Status Sebagai</label>
                        <select class="input pr-12 w-full border col-span-4" name="cbostatustki" id="cbostatustki" required>
											<?php
																	$sql="select id, keterangan FROM tbstatus_wali";
			       														$sql_row=mysqli_query($koneksi,$sql);
			       														while($sql_res=mysqli_fetch_assoc($sql_row))	
																		{
																?>
																<option value="<?php echo $sql_res["id"]; ?>"><?php echo $sql_res["keterangan"]; ?></option>
			       													<?php
			       														}
			       													?>					
											</select>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt4 = $("#txtnama1").val();
		var _txt5 = $("#cbopekerjaan1").val();		
		var _txt6 = $("#txtalamat1").val();
		var _txt7 = $("#txtnegara").val();		
		var _txt8 = $("#cbostatustki").val();
					
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8;
		wi=window.open("s_tki_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
