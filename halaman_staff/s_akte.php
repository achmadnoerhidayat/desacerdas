...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='18'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
										FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='18'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Pengantar Akte Kelahiran Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='18' and 
		...
		$title="SURAT PENGANTAR AKTE KELAHIRAN ".$txtnama;		
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							(nomor_surat, tgl_surat, title_surat, nama, nik, id_jk, tempat_lhr, 
							tgl_lhr, kd_pekerjaan, alamat, id_kel, id_kategori, id_status_kawin, id_agama, fld_keperluan) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$txtnama','','$cbojk','$txttempatlhr',
							'$txttgl','$cbopekerjaan','$txtalamat','$cbokel','18', '$cbostatus', '$cboagama','$cbokeperluan')");	
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Pengantar Akte Kelahiran</h2>
    </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt6 = $("#cbostatus").val();
		var _txt7 = $("#cbopekerjaan").val();
		var _txt8 = $("#txtalamat").val();		
		var _txt9 = $("#cbokeperluan").val();
		
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9;
		wi=window.open("s_akte_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
