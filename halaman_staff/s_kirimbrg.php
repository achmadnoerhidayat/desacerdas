...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='20'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
		...
		echo"<script>window.alert('Pengaturan Surat Pengantar Kirim Barang Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$txttgl_mulai = ubahformatTgl($_POST['date_of_birth1']);
		$txttgl_selesai = ubahformatTgl($_POST['date_of_birth2']);		
		...
		$txtnmbrg= $_POST['package_name'];
		$txtdigunakan= $_POST['for_use'];
		$txtditerangkan= $_POST['note'];		
		...
		$title="SURAT PENGANTAR KIRIM BARANG ".$txtnama;

		
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							(nomor_surat, tgl_surat, title_surat, nama, nik, id_jk, tempat_lhr, 
							tgl_lhr, kd_pekerjaan, alamat, id_kel, id_kategori, id_status_kawin, id_agama, 
							fld_krmbrg1,fld_krmbrg2,fld_krmbrg3,fld_krmbrg4,fld_krmbrg5) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$txtnama','','$cbojk','$txttempatlhr',
							'$txttgl','$cbopekerjaan','$txtalamat','$cbokel','20', '$cbostatus', '$cboagama',
							'$txtnmbrg','$txtdigunakan','$txtditerangkan','$txttgl_mulai','$txttgl_selesai')");	
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Pengantar Kirim Barang</h2>
    </div>
            ...
            <div class="flex">
                <div class="flex-row items-center w-1/3 mr-2">
                    <div class="w-full pb-3 border-b border-gray-300">
                        <h4 class="mr-auto text-lg font-medium">Biodata Warga</h4>
                    </div>
                ...
                <div class="flex-row items-center w-1/3 mx-2">
                    <div class="w-full pb-3 border-b border-gray-300">
                        <h4 class="mr-auto text-lg font-medium">Data Paket Barang</h4>
                    </div>

                    <div class="w-full mt-4">
                        <label class="block mb-2">Nama Barang</label>
						<input type="text" class="input w-full border mt-2" name="package_name" id="package_name" autocomplete="off" required>						
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-2 block">Digunakan untuk?</label>
						<input type="text" class="input w-full border mt-2" name="for_use" id="for_use" autocomplete="off" required>
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-2 block">Diterangkan bahwa...</label>
						<input type="text" class="input w-full border mt-2" name="note" id="note" autocomplete="off" required>
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-2 block">Masa Berlaku</label>
                        <div class="flex items-center">
                            <input name="date_of_birth1" id="date_of_birth1" data-single-mode="true" class="datepicker input w-1/2 border block mr-2">
                            <span class="mx-2">s/d</span>
                            <input name="date_of_birth2" id="date_of_birth2" data-single-mode="true" class="datepicker input w-1/2 border block ml-2">
                        </div>
                    </div>
                </div>
            </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt9 = $("#package_name").val();
		var _txt10 = $("#for_use").val();
		var _txt11 = $("#note").val();
		var _txt12 = $("#date_of_birth1").val();	
		var _txt13 = $("#date_of_birth2").val();	
					
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9 + "&txt10=" + _txt10 + "&txt11=" + _txt11 + "&txt12=" + _txt12 + "&txt13=" + _txt13;
		wi=window.open("s_kirimbrg_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
