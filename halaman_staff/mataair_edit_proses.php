<?php 
	...
	if(!empty($_FILES["gambar"]["tmp_name"])){
		...
		//if ($size < 5000000 and ($type =='image/jpeg' or $type =='image/jpg' or $type == 'image/png')) {
			...
			mysqli_query($koneksi,"UPDATE tbmata_air SET 
					nama_mata_air='$txtnm', luas='$txtluas', kapasitas='$txtdalam', 
					kondisi='$cbokondisi', pemanfaatan='$txtpotensi', 
					file_photo='$foto', file_name='$name' 
					WHERE id='$id'");
			echo"<script>window.alert('Data Berhasil Disimpan!');window.location=('mataair.php');</script>";
		//}else{
