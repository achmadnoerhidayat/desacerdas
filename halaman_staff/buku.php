...
session_start();
	...
	$id_perpustakaan=$_GET['id']; 
	$cari_kd=mysqli_query($koneksi,"SELECT nm_perpustakaan FROM tbperpustakaan WHERE id_perpustakaan='$id_perpustakaan'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$nm_perpustakaan=$tm_cari['nm_perpustakaan'];																									
?>
...
<?php include "menu_perpustakaan.php"; ?>
            ...
            <div class="content">
                ...
                <div class="top-bar">
					...
					<a href="perpustakaan.php" class="">Perpustakaan</a>
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="" class="breadcrumb--active">Show</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
						...
						<div class="intro-y box p-5">
							<form action="" method="post">
								<div class="flex items-center">
									<input class="input w-3/4 border" placeholder="Judul Buku / Code Buku" id="txtcari" name="txtcari" autocomplete="off">	
									<button type="submit" id="tambah" name="tambah" class="button w-1/4 ml-5 bg-theme-1 text-white">Cari</button>
									<a href="buku.php?id=<?php echo $id_perpustakaan; ?>" class="button w-1/4 ml-5 bg-theme-6 text-white">Reset</a>
								</div>
							</form>
						</div>
					</div>
								...
								<table class="table table-report -mt-2">
									<thead>
										<tr>
											...
											<th class="text-center whitespace-no-wrap">KODE RAK</th>
											...
											<?php 
												...
												if(isset($_POST['tambah'])){	
													...
													if ($txtcari<>'') {
														$result = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan' and 
														(nm_buku like '%$txtcari%' or kd_buku like '%$txtcari%')");
													}
													if ($txtcari=='') {
														$result = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan'");
													}
												} else {
													$result = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan'");
												}
												...
												if(isset($_POST['tambah'])){	
													$txtcari = $_POST['txtcari'];
													if ($txtcari<>'') {
														$query = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan' and 
																						(nm_buku like '%$txtcari%' or kd_buku like '%$txtcari%') LIMIT $mulai, $halaman")or die(mysql_error);												
													}
													if ($txtcari=='') {
														$query = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan' LIMIT $mulai, $halaman")or die(mysql_error);													
													}
												} else {
													$query = mysqli_query($koneksi,"SELECT * FROM tbbuku WHERE id_perpustakaan='$id_perpustakaan' LIMIT $mulai, $halaman")or die(mysql_error);
												}
									...
									<tbody>
											<tr>
												<td>
													<a href="" class="font-medium whitespace-no-wrap"><?php echo $data['kd_buku']?></a> 
												</td>
												<td>
													<a href="" class="font-medium whitespace-no-wrap"><?php echo $data['nm_buku']?></a> 
													<div class="text-gray-600 text-xs whitespace-no-wrap"><?php echo $data['deskripsi_buku']?></div>
												</td>												
												<td class="text-center"><?php echo $data['kd_rak']?></td>
