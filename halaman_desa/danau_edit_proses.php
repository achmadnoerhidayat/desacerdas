	...
	if(!empty($_FILES["gambar"]["tmp_name"])){
		...
		//if ($size < 5000000 and ($type =='image/jpeg' or $type =='image/jpg' or $type == 'image/png')) {
			move_uploaded_file($temp, $folder . $name);
				mysqli_query($koneksi,"UPDATE tbdanau SET 
							nama_danau='$txtnm', luas='$txtluas', kedalaman='$txtdalam', 
		...
		mysqli_query($koneksi,"UPDATE tbdanau SET 
							...
							kondisi='$cbokondisi', pemanfaatan='$txtpotensi' 
							WHERE id='$id'");
		echo"<script>window.alert('Data Berhasil Disimpan!');window.location=('danau.php');</script>";
	}
?>