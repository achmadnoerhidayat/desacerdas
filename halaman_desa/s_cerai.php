...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='6'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='6'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Keterangan Cerai Belum ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$txttglcerai = ubahformatTgl($_POST['date_of_birth1']);		
		// = ubahformatTgl($_POST['txttgllahir']); 
		// = ubahformatTgl($_POST['txttglcerai']); 		
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='6' and 
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							nama, nik, tempat_lhr, tgl_lhr, id_jk, kd_pekerjaan, alamat, id_agama, 
							nama_pasutri, tgl_cerai, fld_umur1) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','6',
							'$txtnama','$txtnik','$txttempatlhr','$txttgl','$cbojk','$cbopekerjaan','$txtalamat', '$cboagama', 
							'$txtnmpasutri','$txttglcerai','$txtumur')");	
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Keterangan Cerai</h2>
    </div>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>
                ...
                <div class="flex-row items-center w-1/2 ml-2">
                    <div class="w-full border-b border-gray-300 pb-3 mt-4">
                        <h4 class="text-lg font-medium mr-auto">Data Perceraian</h4>
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-2 block">Nama Mantan Suami/Istri</label>
                        <input type="text" class="input w-full border mt-2" id="txtnmpasutri" name="txtnmpasutri" autocomplete="off" required>
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-2 block">Tanggal Cerai</label>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt1 = $("#date_of_birth1").val();
		var _txt2 = $("#txtumur").val();		
		var _txt3 = $("#txtnmpasutri").val();
		...
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9 + "&txt11=" + _txt11 + "&txt12=" + _txt12;
		wi=window.open("s_cerai_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
