...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='3'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
		FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='3'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Keterangan Miskin Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='3' and 
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Keterangan Miskin</h2>
    </div>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>
                    ...
                    <div class="w-full mt-4">
                        <label class="mb-2 block">Untuk Keperluan / Mendapat Bantuan</label>
<textarea class="input w-full border mt-2" id="txtkeperluan" name="txtkeperluan" required></textarea>
                    </div>
                </div>
            </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var queryString = "?txt1=" + _txt1 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9 + "&txt11=" + _txt11 + "&txt12=" + _txt12;
		wi=window.open("s_miskin_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
