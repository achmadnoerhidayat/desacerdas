...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='16'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
						FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='16'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Pengantar Kartu Tanda Penduduk Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}	
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$cbolayanan = $_POST['cbolayanan'];
		$cbokeperluan = $_POST['cbokeperluan'];
		$txtket = $_POST['txtket'];
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='16' and 
		...
		$title="SURAT PENGANTAR KARTU TANDA PENDUDUK ".$txtnama;
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							'$txttgl','$cbostatus','$cboagama','$cbopekerjaan','$txtalamat','$cbokel','16',
							'$cbolayanan','$cbokeperluan','$txtket')");	
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8 justify-end">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Pengantar Kartu Tanda Penduduk</h2>

    <form action="" method="POST">
        <input type="hidden" name="txtkdkel" value="<?php echo $kd_kel; ?>"/>
        <div class="w-full mt-4">
                        <select class="input pr-12 w-full border col-span-4" name="cbolayanan" id="cbolayanan" required>
											<?php
																	$sql="select layanan FROM tblayanan_ktp order by id";
			       														$sql_row=mysqli_query($koneksi,$sql);
			       														while($sql_res=mysqli_fetch_assoc($sql_row))	
																		{
																?>
																<option value="<?php echo $sql_res["layanan"]; ?>"><?php echo $sql_res["layanan"]; ?></option>
			       													<?php
			       														}
			       													?>					
											</select>
        ...
        <div class="p-5 mt-2 box intro-y">
            <div class="flex">
                ...
                <div class="flex-row items-center w-1/3 mx-2">
                    <div class="w-full pb-3 border-b border-gray-300">
                        <h4 class="mr-auto text-lg font-medium">Data Kebutuhan</h4>
                    </div>

                    <div class="w-full mt-4">
                        <label class="mb-4 block">Keperluan</label>
                        <select class="input pr-12 w-full border col-span-4" name="cbokeperluan" id="cbokeperluan" required>
											<?php
																	$sql="select keperluan FROM tbkeperluan order by id";
			       														$sql_row=mysqli_query($koneksi,$sql);
			       														while($sql_res=mysqli_fetch_assoc($sql_row))	
																		{
																?>
																<option value="<?php echo $sql_res["keperluan"]; ?>"><?php echo $sql_res["keperluan"]; ?></option>
			       													<?php
			       														}
			       													?>					
											</select>
...
<textarea class="input w-full border mt-2" id="txtket" name="txtket" required></textarea>
                    </div>
                </div>
            </div>
...
function popUp(){
	if (wi) wi.close();
		var _txt1 = $("#cbolayanan").val();
		var _txt2 = $("#cbokeperluan").val();		
		var _txt3 = $("#txtket").val();
		var _txt4 = $("#txtnama").val();
		var _txt6 = $("#txttempatlhr").val();
		var _txt7 = $("#date_of_birth").val();
		var _txt8 = $("#cbojk").val();
		var _txt9 = $("#cboagama").val();
		var _txt10 = $("#cbostatus").val();
		var _txt11 = $("#cbopekerjaan").val();
		var _txt12 = $("#txtalamat").val();		
			
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9 + "&txt10=" + _txt10 + "&txt11=" + _txt11 + "&txt12=" + _txt12;
		wi=window.open("s_ktp_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
