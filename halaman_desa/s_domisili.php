...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='1'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
						FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='1'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Keterangan Domisili Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}	
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='1' and 
		...
		$title="SURAT KETERANGAN DOMISILI ".$txtnama;
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$txtnama','$txtnik','$cbojk','$txttempatlhr',
							'$txttgl','$cbostatus','$cboagama','$cbopekerjaan','$txtalamat','$cbokel','1',
...
<div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Keterangan Domisili</h2>
    </div>
...
<textarea class="input w-full border mt-2" id="txtalamattjwb" name="txtalamattjwb" required autocomplete="off"></textarea>
                    </div>
                </div>
...
function popUp(){
	if (wi) wi.close();
		...
		wi=window.open("s_domisili_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
...
var url = "s_domisili_preview.php";
