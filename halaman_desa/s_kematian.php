...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='7'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='7'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Keterangan Kematian Belum ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
...
function ubahformatTgl($tanggal) {
		...
		$txttglmeninggal = ubahformatTgl($_POST['date_of_birth2']);		
		
		$cbokel = $_POST['txtkdkel'];
		$txtnama = $_POST['txtnama'];
		$title="SURAT KETERANGAN KEMATIAN ".$txtnama;		
		...
		$txttempat_meninggal = $_POST['txttempat_meninggal'];	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='7' and 
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							nama1, nik1, tempat_lhr1, tgl_lhr1, id_jk1, id_agama1, kd_pekerjaan1, alamat1,
							tgl_meninggal, tempat_usaha) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','7',
							'$txtnama','$txtnik','$txttempatlhr','$txttgl','$cbojk','$cboagama','$cbopekerjaan','$txtalamat',
							'$txtnama1','$txtnik1','$txttempatlhr1','$txttgl1','$cbojk1','$cboagama1','$cbopekerjaan1','$txtalamat1',
							'$txttglmeninggal','$txttempat_meninggal')");	
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Keterangan Kematian</h2>
    </div>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>
                    ...
                    <div class="flex mt-4">
                        <div class="w-1/2 mr-2">
                            <label class="mb-2 block">Tempat Meninggal</label>
                            <input type="text" class="input w-full border mt-2" id="txttempat_meninggal" name="txttempat_meninggal" autocomplete="off" required>
                        </div>

                        <div class="w-1/2 ml-2">
                            <label class="mb-2 block">Tanggal Meninggal</label>
<input name="date_of_birth2" id="date_of_birth2" class="datepicker input w-full border block mx-auto" data-single-mode="true" >
                ...
                <div class="flex-row items-center w-1/2 ml-2">
                    <div class="w-full border-b border-gray-300 pb-3">
                        <h4 class="text-lg font-medium mr-auto">Biodata Pasangan</h4>
                    </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt9 = $("#txtnama1").val();
		var _txt10 = $("#txtnik1").val();		
		var _txt11 = $("#txttempatlhr1").val();
		var _txt12 = $("#date_of_birth1").val();
		var _txt13 = $("#cbojk1").val();
		var _txt14 = $("#cboagama1").val();
		var _txt15 = $("#cbopekerjaan1").val();
		var _txt16 = $("#txtalamat1").val();		
		var _txt17 = $("#date_of_birth2").val();
		var _txt18 = $("#txttempat_meninggal").val();
		...
		wi=window.open("s_kematian_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
