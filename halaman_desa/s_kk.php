...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='17'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan 
						FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='17'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Pengantar Kartu keluarga Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}	
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='17' and 
		...
		$title="SURAT PENGANTAR KARTU KELUARGA ".$txtnama;
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							'$txttgl','$cbostatus','$cboagama','$cbopekerjaan','$txtalamat','$cbokel','17',
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
    ...
    <div class="intro-y flex items-center mt-8 justify-end">
        <h2 class="text-lg font-medium mr-auto">Layanan Surat - Form Buat Surat Pengantar Kartu Keluarga</h2>
...
function popUp(){
	if (wi) wi.close();		
		...
		wi=window.open("s_kk_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
