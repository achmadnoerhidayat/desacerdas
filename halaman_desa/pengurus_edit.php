...
session_start();
	...
	$id=$_GET['kd'];
	$cari_kd=mysqli_query($koneksi,"SELECT id_rt, nama, id_jabatan, file_photo FROM tbpengurus_rt WHERE id='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$id_rt=$tm_cari['id_rt'];
	$nama=$tm_cari['nama'];
	$id_jabatan=$tm_cari['id_jabatan'];
	$photo_pengurus=$tm_cari['file_photo'];
	if($photo_pengurus=='') {
		$photo_pengurus="dist/images/logo.svg";
	}
	
	$cari_kd=mysqli_query($koneksi,"SELECT tbrw.id_dukuh FROM tbrt, tbrw WHERE tbrt.id_rw=tbrw.id AND tbrt.id_rt='$id_rt'");
	$tm_cari=mysqli_fetch_array($cari_kd);
																...
																<div class="mt-3">
																	...
																	<div>
																		<label><b>Upload Foto Pengurus RT</b>&nbsp;
																															<a href="javascript:void(0);" onClick="newPopup('peta_detail.php?img=<?php echo $photo_pengurus; ?>');"><?php echo $photo_pengurus; ?></a>
																															</label>
