...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='26'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='26'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Izin Guna Jalan Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$title="SURAT IZIN GUNA JALAN ".$txtnama;	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='26' and 
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','26',
...
<div class="flex items-center mt-8 intro-y">
        <h2 class="mr-auto text-lg font-medium">Layanan Surat - Form Buat Surat Izin Guna Jalan</h2>
    </div>
...
function popUp(){
	if (wi) wi.close();
		...
		wi=window.open("s_guna_jalan_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
