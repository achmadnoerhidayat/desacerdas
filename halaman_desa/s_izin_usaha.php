...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='25'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='25'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Izin Usaha Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	
	if(isset($_POST['btnsimpan'])) {			
		$cbokel = $_POST['txtkdkel'];
		$txtnama = $_POST['txtnama'];
		$title="SURAT IZIN USAHA ".$txtnama;	
	
		$cbopekerjaan = $_POST['cbopekerjaan'];
		$txtalamat = $_POST['txtalamat'];	

		$txtnamausaha = $_POST['txtnamausaha'];
		$txtbergerak = $_POST['txtbergerak'];		
		$txtalamat_usaha = $_POST['txtalamat_usaha'];		
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='25' and 
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','25',
							'$txtnama','$cbopekerjaan','$txtalamat',
							'$txtnamausaha','$txtbergerak','$txtalamat_usaha')");
...
<div class="flex items-center mt-8 intro-y">
        <h2 class="mr-auto text-lg font-medium">Layanan Surat - Form Buat Surat Izin Usaha</h2>
    </div>
        ...
        <div class="p-5 mt-2 box intro-y">
            <div class="flex">
                <div class="flex-row items-center w-1/3 mr-2">
                    ...
                    <div class="w-full mt-4">
                        <label class="block mb-4">Pekerjaan</label>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>						
                ...
                <div class="flex-row items-center w-1/3 mx-2">
                    <div class="w-full pb-3 border-b border-gray-300">
                        <h4 class="mr-auto text-lg font-medium">Data Usaha</h4>
                    </div>

                    <div class="w-full mt-4">
                        <label class="block mb-2">Nama Usaha</label>
                        <input type="text" class="input w-full border mt-2" id="txtnamausaha" name="txtnamausaha" autocomplete="off" required>
                    </div>

                    <div class="w-full mt-4">
                        <label class="block mb-2">Bergerak di bidang</label>
                        <input type="text" class="input w-full border mt-2" id="txtbergerak" name="txtbergerak" autocomplete="off" required>
                    </div>

                    <div class="w-full mt-4">
                        <label class="block mb-2">Alamat tempat usaha</label>
<textarea class="input w-full border mt-2" id="txtalamat_usaha" name="txtalamat_usaha" required></textarea>
                    </div>
                </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt4 = $("#txtnamausaha").val();
		var _txt5 = $("#txtbergerak").val();
		var _txt6 = $("#txtalamat_usaha").val();
			
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6;
		wi=window.open("s_izin_usaha_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
