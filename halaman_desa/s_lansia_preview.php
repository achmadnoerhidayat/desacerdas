<?php
	...
	$fld_penyakit=$_GET['txt13'];	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT no_pengaturan 
						FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='12'");
		$tm_cari=mysqli_fetch_array($cari_kd);
...
// === Header ==============									
	$cari_kd=mysqli_query($koneksi,"SELECT 
									...
									WHERE id_kel='$kd_kel' and id_kategori='12'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	...
	$html = '<head>
			...
			<div style="text-align: center; margin: 30pt 0;">
				<div style="text-decoration: underline; font-weight: bold;">SURAT KETERANGAN LANSIA</div>		
				<div>'.$no_surat.'</div>				
			</div>
...
<!-- Begin : Isi Surat -->
        ...
        <div style="text-indent: 40px; margin-top: 10pt;">
            <span>
                Berdasarkan pengamatan Kami, yang bersangkutan benar-benar penduduk yang berdomisili di 
				Desa '.$fld_nama_kel.', '.$fld_nama_kec.' dan terdaftar dalam register kependudukan Kami dengan 
				NIK '.$nik.', nama tersebut diatas benar telah lanjut usia dan tidak bisa meninggalkan tempat 
				karena sakit '.$fld_penyakit.'.
            </span>
        </div>
        <div style="text-indent: 40px; margin-top: 10pt;">
            <span>
                Demikian Surat Keterangan Lansia ini dikeluarkan kepada yang bersangkutan untuk dipergunakan sebagaimana mestinya.
            </span>
        </div>
...
$dompdf->stream('surat_keterangan_lansia.pdf',array("Attachment"=>0));
?>
