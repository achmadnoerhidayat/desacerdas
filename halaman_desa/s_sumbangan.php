...
session_start();
	...
	$data = mysqli_query($koneksi,"SELECT * FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='24'");
	$cek = mysqli_num_rows($data);
	if($cek > 0){	
		$cari_kd=mysqli_query($koneksi,"SELECT id, alamat, tempat, jabatan, nama, nip, no_pengaturan FROM tbpengaturan_surat WHERE id_kel='$kd_kel' and id_kategori='24'");
		$tm_cari=mysqli_fetch_array($cari_kd);
		$nip_jwb=$tm_cari['nip'];
		$nm_jwb=$tm_cari['nama'];
		...
		echo"<script>window.alert('Pengaturan Surat Izin Minta Sumbangan Belum Ada. Silahkan Buat Pengaturan Suratnya.');window.location=('pengaturan_surat.php');</script>";		
	}
	...
	if(isset($_POST['btnsimpan'])) {	
		...
		$title="SURAT IZIN MINTA SUMBANGAN ".$txtnama;	
		...
		$cari_kd=mysqli_query($koneksi,"SELECT count(nomor_surat) as total 
						FROM tbbuatsurat WHERE id_kel='$kd_kel' and id_kategori='24' and 
		...
		mysqli_query($koneksi,"INSERT INTO tbbuatsurat 
							...
							fld_kegunaan, tgl_acara, fld_tempat) 
							VALUES 
							('$txtno','$tgl_skr_eng','$title','$cbokel','24',
...
<div class="flex items-center mt-8 intro-y">
        <h2 class="mr-auto text-lg font-medium">Layanan Surat - Form Buat Surat Izin Minta Sumbangan</h2>
    </div>
...
<textarea class="input w-full border mt-2" id="txtalamat" name="txtalamat" required></textarea>
                ...
                <div class="flex-row items-center w-1/3 mx-2">
                    ...
                    <div class="w-full mt-4">
                        ...
                        <input type="text" class="input w-full border mt-2" id="txtuntuk" name="txtuntuk" autocomplete="off" required>
                    </div>
...
function popUp(){
	if (wi) wi.close();
		...
		var _txt9 = $("#txtuntuk").val();
		var _txt10 = $("#txttempat").val();

			
		var queryString = "?txt1=" + _txt1 + "&txt2=" + _txt2 + "&txt3=" + _txt3 + "&txt4=" + _txt4 + "&txt5=" + _txt5 + "&txt6=" + _txt6 + "&txt7=" + _txt7 + "&txt8=" + _txt8 + "&txt9=" + _txt9 + "&txt10=" + _txt10;
		wi=window.open("s_sumbangan_preview.php" + queryString,"","width=700,height=600,location=no,scrollbars=no,status=no");
}
//-->
</script>
