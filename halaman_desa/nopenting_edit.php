...
session_start();
	...
	$id=$_GET['id'];
	$cari_kd=mysqli_query($koneksi,"SELECT * FROM tbnopenting WHERE id_nopenting='$id'");
	$tm_cari=mysqli_fetch_array($cari_kd);
	$nama=$tm_cari['nama'];
	$id_kategori=$tm_cari['id_kategori'];
	$notlp=$tm_cari['notlp'];
?>
...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="index.php" class="">Home</a> 
					...
					<a href="nopenting.php" class="">No. Penting</a>
					<i data-feather="chevron-right" class="breadcrumb__icon"></i>
					<a href="" class="breadcrumb--active">Edit Nomor Penting</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 lg:col-span-12">
                        ...
                        <div class="intro-y box"> 
							...
							<form class="form-horizontal" action="nopenting_edit_save.php" method="post">
								<input type="hidden" name="txtid" value="<?php echo $id; ?>"/>
								<div class="p-5" id="input">
									<div class="preview">
										<div class="mt-3">
											<input type="text" class="input w-full border mt-2" id="txtnama" name="txtnama" 
											autocomplete="off" value="<?php echo $nama; ?>">
										</div>	
										<div class="mt-3">
											<select class="input pr-12 w-full border col-span-4" id="cbokat" name="cbokat" required>
												<option value="">Kategori</option>
												<?php
													$q = mysqli_query($koneksi,"select id, nama FROM tbkategori_nopenting");
													while ($row1 = mysqli_fetch_array($q)){
														$k_id           = $row1['id'];
														$k_opis         = $row1['nama'];
												?>
												<option value='<?php echo $k_id; ?>' <?php if ($k_id == $id_kategori){ echo 'selected'; } ?>><?php echo $k_opis; ?></option>
												<?php
													}
												?>
											</select>
										...
										<div class="mt-3">
											<input type="text" class="input w-full border mt-2" id="txttlp" name="txttlp" 
											autocomplete="off" value="<?php echo $notlp; ?>">
										</div>										
